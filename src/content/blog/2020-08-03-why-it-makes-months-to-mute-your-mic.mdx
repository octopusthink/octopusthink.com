...
authors: [matt]
tags: [development]
title: Why it takes months to mute your mic
summary: |
  For a relatively simple app, Mic Drop took us quite awhile to build—and that's not even counting all the time we've spent on marketing! It turns out that even a very "simple" app has a lot more going on than you'd think.

  I wanted to explore what's beneath the surface and share some of what we've learned from our first foray into macOS app development, from a (mostly) technical perspective.
...
In November. ([We released Mic Drop in April.](/blog/2020-04-29-say-hello-to-mic-drop)) 😅
...
## Borrowing from open source & sharing back

We try to avoid reinventing the wheel where it make sense. We built on existing open-source libraries to handle [recording keyboard shortcuts](https://github.com/octopusthink/KeyHolder). Our [customised versions](https://github.com/octopusthink/Magnet) of these libraries are available for you to use in your own projects.

We've published a [Under the Hood](https://getmicdrop.com/under-the-hood/) page on the Mic Drop website. This is something we'll be doing for all future apps—partly because we're curious, and partly because we're fans of transparency. It allows us an opportunity to explain our choices and gives others a reference for their own projects. 📕

It's also one of the most popular pages on the Mic Drop website, so either other people are curious as well, or they're finding it useful! Either way: it's definitely worth sharing.
